...
def profile():
    ...
    # total_points = 1
    return render_template("profile.html", user=current_user, pick_table=pick_table, user_points=total_points)
...
def league():
    ...
    return render_template(
        "league.html", u_table=user_table, p_table=pick_table, pl_table=player_table, show_picks=show_picks
    )
...
def pick():
    ...
    if session['event_state'] == 'pre':
        if prev_pick is None:
            pick_state = "you have yet to pick.".format(curr_event)
        else:
            pick_state = "you have already picked, but can modify your pick free of charge.".format(curr_event)
    else:
        if (prev_pick is None) and (current_user.strikes_remaining):
            pick_state = "the tourney has started and you have not picked, but you have a strike. Picking now will use this up.".format(curr_event, int(current_user.strikes_remaining))
            button_text = "Pick and Use Strike"
        else:
            pick_state = "mate, the {} has started and you have either made your pick, or don't have any strikes left.".format(curr_event)
            button_state = False

    print (session)
    return render_template(
        "pick.html", avail=avail_picks, event=curr_event, user=current_user.name, pick_text=pick_state, button=button_state, submit_text=button_text
    )
